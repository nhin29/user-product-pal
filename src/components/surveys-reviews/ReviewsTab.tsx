...
export default function ReviewsTab() {
  ...
  const handleDelete = async (ids: string[]) => {
    try {
      ...
      if (selectedReview && ids.includes(selectedReview.id)) setSelectedReview(null);
    } catch {
    ...
    <>
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-8">
        <Card>
          ...
          <CardContent>{isLoading ? <Skeleton className="h-8 w-16" /> : <div className="text-3xl font-bold">{reviews.length}</div>}</CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Rating Distribution</CardTitle></CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="space-y-2">{[1, 2, 3].map((i) => <Skeleton key={i} className="h-3 w-full" />)}</div>
            ) : (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {ratingFilter === "all" ? "User reviews sorted by most recent" : `Showing ${filteredReviews.length} review(s) with ${ratingFilter} star${Number(ratingFilter) !== 1 ? "s" : ""}`}
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <Select value={ratingFilter} onValueChange={(v) => { setRatingFilter(v); setCurrentPage(1); setSelectedIds(new Set()); }}>
                <SelectTrigger className="h-9 w-[140px]"><SelectValue placeholder="Filter rating" /></SelectTrigger>
              ...
              {selectedIds.size > 0 && (
                <Button variant="destructive" size="sm" onClick={() => setConfirmDeleteIds(Array.from(selectedIds))}>
            ...
            <>
              <div className="flex items-center gap-3 mb-3 pb-3 border-b">
                ...
                <span className="text-sm text-muted-foreground">
                  {selectedIds.size > 0 ? `${selectedIds.size} of ${reviews.length} selected` : "Select all on page"}
                </span>
              </div>
              <div className="space-y-3">
                {paginatedReviews.map((review) => (
                  <div key={review.id} className="flex items-start gap-3 p-4 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                    <div className="pt-0.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox checked={selectedIds.has(review.id)} onCheckedChange={() => toggleSelect(review.id)} />
                    </div>
                    <div className="flex items-start gap-4 flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedReview(review)}>
                      {review.avatar_url ? (
                        <img src={review.avatar_url} alt={review.display_name || "User"} loading="lazy" decoding="async" className="h-10 w-10 rounded-full object-cover ring-1 ring-border" />
                      ) : (
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 ring-1 ring-border shrink-0">
                          <span className="text-sm font-semibold text-primary">{review.display_name?.[0]?.toUpperCase() || "?"}</span>
                        </div>
                      )}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                {selectedReview.avatar_url ? (
                  <img src={selectedReview.avatar_url} alt={selectedReview.display_name || "User"} loading="lazy" decoding="async" className="h-12 w-12 rounded-full object-cover ring-1 ring-border" />
                ) : (
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 ring-1 ring-border">
                    <span className="text-base font-semibold text-primary">{selectedReview.display_name?.[0]?.toUpperCase() || "?"}</span>
                  </div>
                )}
                <div>
              ...
              {selectedReview.comment ? (
                <div className="rounded-lg bg-muted/50 border p-4">
                  <p className="text-sm text-foreground whitespace-pre-wrap leading-relaxed">{selectedReview.comment}</p>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground italic">No comment provided.</p>
              )}
              <div className="flex justify-end pt-2">
                <Button variant="destructive" size="sm" onClick={() => setConfirmDeleteIds([selectedReview.id])}>
      ...
      <AlertDialog open={confirmDeleteIds.length > 0} onOpenChange={(open) => !open && setConfirmDeleteIds([])}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {confirmDeleteIds.length > 1 ? `${confirmDeleteIds.length} reviews` : "this review"}?</AlertDialogTitle>
            <AlertDialogDescription>This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => handleDelete(confirmDeleteIds)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90" disabled={deleteReviews.isPending}>
              {deleteReviews.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
