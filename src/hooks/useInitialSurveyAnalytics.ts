...
// Q2 - why_peelkit (single string in DB)
...
// Q3 - design_goal (single string in DB, stores goal-type answers)
...
// Q6 - sales_channels (single string in DB)
...
// Q7 - problems_before (array in DB)
...
// Q8 - primary_goal (single string in DB)
...
function countSingleField(
  ...
  responses.forEach((r) => {
    const val = r[field];
    if (typeof val === "string" && val in labels) {
      counts[val] += 1;
    }
  });
  return counts;
