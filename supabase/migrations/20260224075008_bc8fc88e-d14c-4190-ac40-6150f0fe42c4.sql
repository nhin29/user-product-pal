...
CREATE OR REPLACE FUNCTION public.get_user_analytics_stats(p_user_id uuid)
 RETURNS TABLE(total_clicks bigint, total_copies bigint, total_saves bigint, total_page_views bigint, total_spend_time_seconds bigint, total_generations bigint)
 LANGUAGE sql
...
AS $function$
  SELECT
    ...
    (SELECT COALESCE(SUM(total_seconds), 0)::bigint FROM daily_time_tracking WHERE user_id = p_user_id) AS total_spend_time_seconds,
