...
-- Update get_dashboard_chart_data to filter by is_analytics (with proper casts)
...
LANGUAGE sql SECURITY DEFINER AS $$
  ...
  customer_agg AS (
    ...
    FROM profiles
    WHERE created_at >= p_start_date::timestamptz AND created_at <= p_end_date::timestamptz AND is_analytics = true
  ...
  interaction_agg AS (
    ...
    WHERE created_at >= p_start_date::timestamptz AND created_at <= p_end_date::timestamptz
      AND (user_id IS NULL OR user_id IN (SELECT user_id FROM analytics_users))
    GROUP BY created_at::date
  ),
  bookmark_agg AS (
...
-- Update get_top_products_by_interaction to filter by is_analytics
...
LANGUAGE sql SECURITY DEFINER AS $$
  ...
  FROM (
    ...
    WHERE pi.interaction_type = p_interaction_type AND p_interaction_type != 'save'
      AND (pi.user_id IS NULL OR pi.user_id IN (SELECT user_id FROM profiles WHERE is_analytics = true))
    GROUP BY pi.product_id
    UNION ALL
    SELECT b.product_id, count(*) AS interaction_count
    FROM bookmarks b
    WHERE p_interaction_type = 'save'
      AND b.user_id IN (SELECT user_id FROM profiles WHERE is_analytics = true)
